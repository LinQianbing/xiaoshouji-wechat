...
function talkLevelRule(level) {
  ...
  if (n <= 2) {
    return [
      ...
      "话多可以，但每一条都必须接着当前用户消息或当前对话气氛说。不要为了凑气泡主动换话题。",
    ].join("\n");
  }
  if (n <= 4) {
    return [
  ...
  if (n <= 6) {
    return [
      ...
      "如果要多发几条，可以用补充、撒娇、吐槽、追问、情绪回应，但不要突然转到长期记忆里的固定话题。",
    ].join("\n");
  }
  if (n <= 8) {
    return [
  ...
  return messages
    ...
    .map((msg) => {
      if (msg.type === "pat") {
        ...
        return `系统动作：${action}。这只是微信动作事件，不是普通聊天文本；不要复述拍一拍文案。`;
      }
...
function recalledMessagesText(messages = [], roleName = "TA", range = null) {
  if (!range && !messages.length) return "未开启旧聊天检索：用户当前没有明显在问旧事。";
...
export function buildChatPrompt({ role, settings, mode, memories, recentMessages, recalledMessages = [], recalledRange = null, userText }) {
  ...
  const system = [
    ...
    "话多可以，但每一条都必须接着当前用户消息或当前对话气氛说。不要为了凑气泡主动换话题。",
    "如果要多发几条，可以用补充、撒娇、吐槽、追问、情绪回应，但不要突然转到长期记忆里的固定话题。",
    "除非用户当前消息直接相关，否则不要主动提奶茶、歌曲、学习奖励、喝水、睡觉等旧记忆。",
    ...
    "长期记忆里如果标着“还没完全放下/还没后续”，说明这件事像真人心里挂着的旧事；只有用户当前消息、情绪或关键词直接相关时才自然提一句，不要机械翻旧账。",
    "长期记忆里如果标着“TA的感受”，那是你这个联系人对你们互动留下的感觉；它只影响语气和亲疏，不要直接说“我的记忆里写着”。",
    "只有这次对话明确让你产生了担心、安心、在意、别扭、心疼、失落、着急、放松这类具体感受，才写入 feelingMemoryCandidate；不要脑补暗恋、占有欲、吃醋、内心小剧场或没说完的关系戏。角色设定或当前关系如果明确支持暗恋、恋人、暧昧、吃醋、占有欲，可以按设定和关系进度表现，但不要把关系设定本身重复当成新感受记忆。",
    "feelingMemoryCandidate 必须写清楚触发原因，例如“TA看到用户说最近压力很大后有点担心”；如果只是“更了解用户了”“关系更近了”这类泛泛感受，必须留空。",
  ...
  return [
    {
      ...
      content: [
        ...
        "不要输出解释。只输出合法 JSON。",
        "只有这条朋友圈明确让你产生担心、安心、在意、别扭、心疼、失落、着急、放松这类具体感受，才写入 feelingMemoryCandidate；不要脑补暗恋、占有欲、吃醋或没说完的内心戏。角色设定或当前关系如果明确支持暗恋、恋人、暧昧、吃醋、占有欲，可以按设定和关系进度表现，但不要把关系设定本身重复当成新感受记忆。",
        "feelingMemoryCandidate 必须写清楚触发原因，例如“TA看到用户半夜发压力很大后有点担心”；如果只是“更了解用户了”“关系更近了”，必须留空。",
  ...
  return [
    {
      ...
      content: [
        ...
        "inner_feel 只能保存对最近聊天里明确发生过的事产生的具体感受，必须带触发原因；不要补角色设定或当前关系外的暗恋、占有欲、吃醋或自我攻略式内心戏。",
