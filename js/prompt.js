...
export function buildChatPrompt({ role, settings, mode, memories, recentMessages, recalledMessages = [], recalledRange = null, userText }) {
  ...
  const system = [
    ...
    "只有这次对话明确让你产生了具体感受，才写入 feelingMemoryCandidate；感受强度要跟角色设定和关系进度一致。暗恋、恋人、暧昧、吃醋、占有欲、病娇或偏执如果是设定或关系发展的一部分，可以自然表现；不要脱离设定和当前关系凭空加戏，也不要把关系设定本身重复当成新感受记忆。",
  ...
  return [
    {
      ...
      content: [
        ...
        "不要输出解释。只输出合法 JSON。",
        "只有这条朋友圈明确让你产生具体感受，才写入 feelingMemoryCandidate；感受强度要跟角色设定和关系进度一致。暗恋、恋人、暧昧、吃醋、占有欲、病娇或偏执如果是设定或关系发展的一部分，可以自然表现；不要脱离设定和当前关系凭空加戏，也不要把关系设定本身重复当成新感受记忆。",
  ...
  return [
    {
      ...
      content: [
        ...
        "inner_feel 只能保存对最近聊天里明确发生过的事产生的具体感受，必须带触发原因；暗恋、占有欲、吃醋、病娇或偏执可以来自角色设定和关系进度，但不要脱离依据凭空新增。",
