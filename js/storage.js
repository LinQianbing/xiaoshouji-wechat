...
export function addMemory(roleId, memory) {
  ...
  const normalized = {
    ...
    arousal: Number(memory.arousal ?? Math.min(1, Number(memory.emotionWeight ?? 3) / 5)),
    unresolved: Boolean(memory.unresolved),
